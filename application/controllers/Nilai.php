...
class Nilai extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $allowed = array('admin');
        if (!in_array($this->session->userdata('role'), $allowed)) {
            $this->load->view('nilai/index_mhs', $data);
        } else {
    ...
    public function tambah()
    {
        ...
        $this->form_validation->set_rules('nilai_1', 'Nilai Batas 1', 'required|numeric');
        ...
        if ($this->form_validation->run()) {
            $params = array(
                'nilai_1' => $this->input->post('nilai_1', TRUE),
            ...
            $this->session->set_flashdata('failed', '<div class="alert alert-danger" role="alert">Data gagal disimpan</div>');
    ...
    public function prioritas()
    {
        ...
        if (isset($_POST['check'])) {
            ...
            if ($this->ahp_uji_konsistensi($hasil_tabel_konsistensi)) {
                ...
                $list_data = $this->tampil_data_4($jumlah_matrik_baris, $prioritas, $hasil_tabel_konsistensi, $lambda);
                $data['list_data4'] = $list_data[0];
                $data['list_data5'] = $list_data[1];
                $data['list_data6'] = $this->tampil_data_5($prioritas, $jumlah_kolom, $lambda);


                // update id_nilai dari prodi_kriteria
                $this->load->model('prodi_kriteria_model');
                $prodi_kriteria = $this->prodi_kriteria_model->get_all_prodi_kriteria()->result();
                foreach ($prodi_kriteria as $row) {
                    $params = array(
                        'id_nilai' => $this->get_id_nilai($row->nilai),
                    );
                    $this->prodi_kriteria_model->update_by_id($row->id, $params);
                }
            } else {
    ...
    public function ahp_get_prioritas($matrik_normalisasi) //rata-rata
    {
        $prioritas = array();
    ...
    public function tampil_data_4($jumlah_matrik_baris, $prioritas, $hasil_tabel_konsistensi, $lambda)
    {
        $nilai = $this->nilai_model->get_all_nilai()->result();
...
<tr>
    <td width="100">CR <= 0.1</td>';
        if ($cr <= 0.1) {
            ...
            $this->session->set_flashdata('failure', '<div class="alert alert-danger" role="alert">Nilai perbandingan : TIDAK KONSISTEN</div>');
        }
        $list_data5 .= '
</tr>
</table>';
    ...
    public function tampil_data_5($prioritas, $jumlah_kolom, $lambda)
    {
        ...
        $list_data6 .= '<tr><td></td>';
        foreach ($nilai as $row) {
            $list_data6 .= '<td class="text-center">' . $row->nama . '</td>';
        }
        $list_data6 .= '</tr>';
