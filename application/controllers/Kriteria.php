...
class Kriteria extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $allowed = array('admin');
        if (!in_array($this->session->userdata('role'), $allowed)) {
            $this->load->view('kriteria/index_mhs', $data);
        } else {
    ...
    public function tambah()
    {
        ...
        $this->form_validation->set_rules('kode_kriteria', 'Kode Kriteria', 'required|alpha_numeric_spaces');
        $this->form_validation->set_rules('nama_kriteria', 'nama Kriteria', 'required|alpha');
    ...
    public function ubah($id_kriteria = '')
    {
        ...
        if (empty($data['kriteria'])) {
            ...
            $this->form_validation->set_rules('kode_kriteria', 'Kode Kriteria', 'required|alpha_numeric');
            $this->form_validation->set_rules('nama_kriteria', 'nama Kriteria', 'required|alpha');
    ...
    public function ahp_get_prioritas($matrik_normalisasi)
    {
        ...
        for ($i = 0; $i < count($matrik_normalisasi); $i++) {
            ...
            $prioritas[$i] = number_format($prioritas[$i] / count($matrik_normalisasi), 5); //Matriks Nilai Kriteria (Normalisasi) kolom prioritas
        }
        return $prioritas;
    }
    ...
    public function tampil_data_5($prioritas, $jumlah_kolom, $lambda)
    {
        ...
        $list_data6 .= '<tr><td></td>';
        foreach ($kriteria as $row) {
            $list_data6 .= '<td class="text-center">' . $row->kode_kriteria . '</td>';
        }
        $list_data6 .= '</tr>';
